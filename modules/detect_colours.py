...
class DetectBlue:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        lower_blue = np.ndarray([120,0,0])
        upper_blue = np.ndarray([150,255,255])
...
class DetectRed:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        # Set upper and lower bounds for colour detection, this is in HSV
        lower_red_1 = np.ndarray([0,0,0])
        upper_red_1 = np.ndarray([30,255,255])

        #need to account for 180 degrees
        lower_red_2 = np.ndarray([180,0,0])
        upper_red_2 = np.ndarray([180,255,255])
